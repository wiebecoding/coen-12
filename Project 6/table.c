 ...
 * Complexity:	O(n^2)
 *
 * Description:	Recursive funciton to sort the given terms.
 */

void quickSort(void **elts,int low, int high, SET *sp){
    assert(elts!=NULL && sp!=NULL);
    if(low>high){
        return;
    }
    int j = partition(elts,low,high,sp);
    quickSort(elts, low, j-1, sp);
    quickSort(elts, j+1, high, sp);
}
 ...
 * Complexity:	O(n)
 * 
 * Description:	Swaping protocal from pivot
 */
